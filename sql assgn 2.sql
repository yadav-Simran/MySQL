...
2. Create the Student table again without the following columns: DeptName, CourseName,
CurrentBatchName, StudentStatus
...
10. Insert 10 records in the student table. DeptId, CourseId, BatchId, StatusId columns should have
some data common with the master table data.
11. Write an inner join between Student and Department to retrieve the department name for all
the students.
12. Write a query to retrieve all the courses where there is no student.
13. Write a query to retrieve all the students having a status value not present in StudentStatus
table.
14. Select the student records with completed status for CSE Department and CSBS2 batch.
*/
