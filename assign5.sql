/*
Assignment-5: Indexing and Views

1. Drop and create Student and Department tables. Don’t create Primary key, unique
key or foreign key constraints on the tables now.
...
4. Create a nonclustered index on name, address columns of the Student table only for
the students whose address is Kolkata.
5. Drop and recreate the nonclustered index. Email and phone columns should be
added as included columns.
...
departname by joining both the tables. WITH SCHEMABINDING option should be
added.
8. Retrieve Studentname and departname from vStudentDept view only for CSE
department.
